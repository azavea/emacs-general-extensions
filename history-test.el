...
(ert-deftest hist/test/compose ()
  (should (equal -9 (hist/get (funcall (hist/compose (-rpartial '- 10) 'abs) (hist/init 1)))))
  (should (equal 4 (hist/get (funcall (hist/compose '(closure ((x . 2) t) (y) (+ x y))) (hist/init 2)))))
)

(ert-deftest hist/test/filter ()
  (let ((h1 '(9 8 7))
        (h2 '(20 19 18))
        (h3 '(15 100 1000))
        (h4 '(1 23)))
